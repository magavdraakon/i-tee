    ...
    if self.state=='running'
      if self.lab_vmt.allow_remote && self.lab_vmt.guacamole_type!="none"
        ...
        if self.g_connection
          ...
          post = Http.post(guacamole_host+"/guacamole/api/tokens", {username: self.lab_user.g_username, password:self.lab_user.g_password})
          # puts post.body
          if post.body && post.body['authToken']
            # get machine url
            uri = GuacamoleConnection.get_url(self.g_connection)
            path = guacamole_host+"/guacamole/#/client/#{uri}"

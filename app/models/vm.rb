  ...
  def start_vm(info=nil)
    ...
    begin
      unless Virtualbox.all_machines.include? name
        ...
        logger.debug "START_VM: Machine does not exist, creating from '#{self.lab_vmt.vmt.image}' #{loginfo}"

        # Create new instance of template
        begin
          current_snapshot=Virtualbox.get_vm_info(self.lab_vmt.vmt.image)['CurrentSnapshotName']
          logger.debug "START_VM: Cloning from snapshot '#{current_snapshot}' #{loginfo}"
          Virtualbox.clone(self.lab_vmt.vmt.image, name, current_snapshot)
        rescue
          logger.debug "START_VM: Cloning #{loginfo}"

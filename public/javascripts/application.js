...
function add_network_to_vmt(el) {
	const n = $(el).parents('.vmt').find('.network').size();
	const p = $(".vmt").index($(el).parents('.vmt'));
	const options = $("#networks").html();
	const innerHtml = [ ];
...
function add_vmt_to_lab() {
	const n = $("#lab_vmts").find('.vmt').size();
	const vmt_template_options = $("#vm_templates").html();
	const g_type_options = $("#g_types").html();
	const innerHtml = [ ];
